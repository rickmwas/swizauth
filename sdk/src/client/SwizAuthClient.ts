...
export class SwizAuthClient {
  private config: SwizAuthConfig;
  private options: SwizAuthClientOptions;
  private storage: Storage;
  private authState: AuthState;
  private listeners: Set<(state: AuthState) => void> = new Set();

  constructor(config: SwizAuthConfig, options: SwizAuthClientOptions = {}) {
    this.config = {
      ...config,
  ...
  private async init(): Promise<void> {
    try {
      ...
      console.error('SwizAuth initialization error:', error);
  ...
  private async loadStoredAuth(): Promise<void> {
    const accessToken = this.storage.getItem('swizauth_access_token');
    const refreshToken = this.storage.getItem('swizauth_refresh_token');
  ...
  public getAccessToken(): string | null {
    return this.storage.getItem('swizauth_access_token');
  }
  ...
  private storeTokens(tokens: AuthTokens): void {
    this.storage.setItem('swizauth_access_token', tokens.accessToken);
    this.storage.setItem('swizauth_refresh_token', tokens.refreshToken);
  }

  private clearAuth(): void {
    this.storage.removeItem('swizauth_access_token');
    this.storage.removeItem('swizauth_refresh_token');
